...
class EditProfile extends React.Component {
  ...
  async componentDidMount() {
    ...
    try {
      const user = await getAuthUser(auth.user._id);
  ...
  handleSubmit = async (event) => {
    ...
    try {
      const updatedUser = await updateUser(this.state._id, this.userData);
      this.setState({ updatedUser, openSuccess: true }, () => {
        setTimeout(() => Router.push(`/profile/${this.state._id}`), 2500);
      });
    } catch (error) {
